...
class gacl_api extends \gacl {
	...
	function consolidated_edit_acl($aco_section_value, $aco_value, $aro_section_value, $aro_value, $return_value) {/*{{{*/
		...
		M()->debug("ACO Section Value: $aco_section_value ACO Value: $aco_value ARO Section Value: $aro_section_value ARO Value: $aro_value Return Value: $return_value");

		$acl_ids = array();

		if (empty($aco_section_value) ) {
			M()->debug("ACO Section Value ($aco_section_value) is empty, this is required!");
			return false;
		}

		if (empty($aco_value) ) {
			M()->debug("ACO Value ($aco_value) is empty, this is required!");
			return false;
		}

		if (empty($aro_section_value) ) {
			M()->debug("ARO Section Value ($aro_section_value) is empty, this is required!");
			return false;
		}

		if (empty($aro_value) ) {
			M()->debug("ARO Value ($aro_value) is empty, this is required!");
			return false;
		}

		if (empty($return_value) ) {
			M()->debug("Return Value ($return_value) is empty, this is required!");
			return false;
		}
		...
		if (is_array($current_acl_ids)) {
			M()->debug("Found current ACL_IDs, counting ACOs");
			...
			foreach ($current_acl_ids as $current_acl_id) {
				...
				M()->debug("Current Count: ".$this->count_all($current_acl_array['aco'])."");

				if ( $this->count_all($current_acl_array['aco']) == 1) {
					M()->debug("ACL ID: $current_acl_id has 1 ACO.");
					...
					if ($current_acl_array['return_value'] == $return_value) {
						M()->debug("ACL ID: $current_acl_id has 1 ACO, and the same return value. No need to modify.");
						return true;
					}
		...
		if (is_array($acl_ids) AND $acl_ids_count > 0) {
			M()->debug("Removing specified ARO from existing ACL.");
			...
			foreach ($acl_ids as $acl_id) {
				...
				if (!$this->shift_acl($acl_id, array($aro_section_value => array($aro_value)) ) ) {
					M()->debug("Error removing specified ARO from ACL ID: $acl_id");
					return false;
				}
			}
		} else {
			M()->debug("Didn't find any current ACLs with a single ACO. ");
		}
		//unset($acl_ids);
    $acl_ids = array();
		unset($acl_ids_count);
		...
		if (is_array($new_acl_ids)) {
			M()->debug("Found new ACL_IDs, counting ACOs");

			foreach ($new_acl_ids as $new_acl_id) {
				//Check to make sure these ACLs only have a single ACO mapped to them.
				$new_acl_array = &$this->get_acl($new_acl_id);
				//showarray($new_acl_array);
				M()->debug("New Count: ".$this->count_all($new_acl_array['aco'])."");
				if ( $this->count_all($new_acl_array['aco']) == 1) {

					M()->debug("ACL ID: $new_acl_id has 1 ACO, append should be able to take place.");
					$acl_ids[] = $new_acl_id;
				}
		...
		if (is_array($acl_ids) AND $acl_ids_count == 1) {
			M()->debug("Appending specified ARO to existing ACL.");

			$acl_id=$acl_ids[0];

			if (!$this->append_acl($acl_id, array($aro_section_value => array($aro_value)) ) ) {
				M()->debug("Error appending specified ARO to ACL ID: $acl_id");
				return false;
			}

			M()->debug("Hot damn, ACL consolidated!");
			return true;
		} elseif($acl_ids_count > 1) {
			M()->debug("Found more then one ACL with a single ACO. Possible conflicting ACLs.");
			return false;
		} elseif ($acl_ids_count == 0) {
			M()->debug("No existing ACLs found, create a new one.");
			...
			if (!$this->add_acl(	array( $aco_section_value => array($aco_value) ),
				...
				M()->debug("Error adding new ACL for ACO Section: $aco_section_value ACO Value: $aco_value Return Value: $return_value");
				return false;
			}

			M()->debug("ADD_ACL() successfull, returning True.");
			return true;
		}

		M()->debug("Returning false.");
		return false;
	}/*}}}*/
	...
	function search_acl($aco_section_value=NULL, $aco_value=NULL, $aro_section_value=NULL, $aro_value=NULL, $aro_group_name=NULL, $axo_section_value=NULL, $axo_value=NULL, $axo_group_name=NULL, $return_value=NULL) {/*{{{*/
		M()->debug("aco_section_value: $aco_section_value aco_value: $aco_value, aro_section_value: $aro_section_value, aro_value: $aro_value, aro_group_name: $aro_group_name, axo_section_value: $axo_section_value, axo_value: $axo_value, axo_group_name: $axo_group_name, return_value: $return_value");
	...
	function append_acl($acl_id, $aro_array=NULL, $aro_group_ids=NULL, $axo_array=NULL, $axo_group_ids=NULL, $aco_array=NULL) {/*{{{*/
		M()->debug("ACL_ID: $acl_id");

		$update = 0;

		if (empty($acl_id)) {
			M()->debug("No ACL_ID specified! ACL_ID: $acl_id");
			return false;
		}
				...
				foreach ($aro_value_array as $aro_value) {
					if ( count($acl_array['aro'][$aro_section_value]) != 0 ) {
						if (!in_array($aro_value, $acl_array['aro'][$aro_section_value])) {
							M()->debug("ARO Section Value: $aro_section_value ARO VALUE: $aro_value");
							$acl_array['aro'][$aro_section_value][] = $aro_value;
							$update=1;
						} else {
							M()->debug("Duplicate ARO, ignoring... ");
						}
			...
			while (list($axo_section_value,$axo_value_array) = @each($axo_array)) {
				foreach ($axo_value_array as $axo_value) {
					if (!in_array($axo_value, $acl_array['axo'][$axo_section_value])) {
						M()->debug("AXO Section Value: $axo_section_value AXO VALUE: $axo_value");
						$acl_array['axo'][$axo_section_value][] = $axo_value;
						$update = 1;
					} else {
						M()->debug("Duplicate AXO, ignoring... ");
					}
			...
			while (list($aco_section_value,$aco_value_array) = @each($aco_array)) {
				foreach ($aco_value_array as $aco_value) {
					if (!in_array($aco_value, $acl_array['aco'][$aco_section_value])) {
						M()->debug("ACO Section Value: $aco_section_value ACO VALUE: $aco_value");
						$acl_array['aco'][$aco_section_value][] = $aco_value;
						$update = 1;
					} else {
						M()->debug("Duplicate ACO, ignoring... ");
					}
				}
			}
		}
		...
		if ($update == 1) {
			M()->debug("Update flag set, updating ACL.");
		...
		M()->debug("Update flag not set, NOT updating ACL.");
		return true;
	}/*}}}*/
	...
	function shift_acl($acl_id, $aro_array=NULL, $aro_group_ids=NULL, $axo_array=NULL, $axo_group_ids=NULL, $aco_array=NULL) {/*{{{*/
			...
			while (list($aro_section_value,$aro_value_array) = @each($aro_array)) {
				foreach ($aro_value_array as $aro_value) {
					...
					if ( count($acl_array['aro'][$aro_section_value]) != 0 ) {
						...
						if ($aro_key !== FALSE) {
							M()->debug("Removing ARO. ($aro_key)");
							unset($acl_array['aro'][$aro_section_value][$aro_key]);
							$update = 1;
						} else {
							M()->debug("ARO doesn't exist, can't remove it.");
						}
					}
		...
		if (is_array($aro_group_ids) AND count($aro_group_ids) > 0) {
			M()->debug("Removing ARO_GROUP_ID's");
			...
			while (list(,$aro_group_id) = @each($aro_group_ids)) {
				...
				if ($aro_group_key !== FALSE) {
					M()->debug("Removing ARO Group. ($aro_group_key)");
					unset($acl_array['aro_groups'][$aro_group_key]);
					$update = 1;
				} else {
					M()->debug("ARO Group doesn't exist, can't remove it.");
				}
			}
		}

		if (is_array($axo_array) AND count($axo_array) > 0) {
			M()->debug("Removing AXO's");
			...
			while (list($axo_section_value,$axo_value_array) = @each($axo_array)) {
				foreach ($axo_value_array as $axo_value) {
					...
					if ($axo_key !== FALSE) {
						M()->debug("Removing AXO. ($axo_key)");
						unset($acl_array['axo'][$axo_section_value][$axo_key]);
						$update = 1;
					} else {
						M()->debug("AXO doesn't exist, can't remove it.");
					}
				}
			}
		}
		...
		if (is_array($axo_group_ids) AND count($axo_group_ids) > 0) {
			M()->debug("Removing AXO_GROUP_ID's");
			...
			while (list(,$axo_group_id) = @each($axo_group_ids)) {
				...
				if ($axo_group_key !== FALSE) {
					M()->debug("Removing AXO Group. ($axo_group_key)");
					unset($acl_array['axo_groups'][$axo_group_key]);
					$update = 1;
				} else {
					M()->debug("AXO Group doesn't exist, can't remove it.");
				}
			}
		}

		if (is_array($aco_array) AND count($aco_array) > 0) {
			M()->debug("Removing ACO's");
			...
			while (list($aco_section_value,$aco_value_array) = @each($aco_array)) {
				foreach ($aco_value_array as $aco_value) {
					...
					if ($aco_key !== FALSE) {
						M()->debug("Removing ACO. ($aco_key)");
						unset($acl_array['aco'][$aco_section_value][$aco_key]);
						$update = 1;
					} else {
						M()->debug("ACO doesn't exist, can't remove it.");
					}
				}
			}
		}
		...
		if ($update == 1) {
			...
			M()->debug("ACOs: ". $this->count_all($acl_array['aco']) ." AROs: ".$this->count_all($acl_array['aro'])."");
			...
			if ( $this->count_all($acl_array['aco']) == 0
				...
				M()->debug("No ACOs or ( AROs AND AXOs AND ARO Groups AND AXO Groups) left assigned to this ACL (ID: $acl_id), deleting ACL.");
	...
	function get_acl($acl_id) {/*{{{*/
		...
		if (!$acl_row) {
			M()->debug("No ACL found for that ID! ACL_ID: $acl_id");
			return false;
		}
		...
		while (list(,$row) = @each($rows)) {
			...
			M()->debug("Section Value: $section_value Value: $value Section: $section ACO: $aco");
		...
		while (list(,$row) = @each($rows)) {
			...
			M()->debug("Section Value: $section_value Value: $value Section: $section ARO: $aro");
		...
		while (list(,$row) = @each($rows)) {
			...
			M()->debug("Section Value: $section_value Value: $value Section: $section AXO: $axo");
	...
	function is_conflicting_acl($aco_array, $aro_array, $aro_group_ids=NULL, $axo_array=NULL, $axo_group_ids=NULL, $ignore_acl_ids=NULL) {/*{{{*/
		...
		if (!is_array($aco_array)) {
			M()->debug('is_conflicting_acl(): Invalid ACO Array.');
			return FALSE;
		}

		if (!is_array($aro_array)) {
			M()->debug('is_conflicting_acl(): Invalid ARO Array.');
			return FALSE;
		}
		...
		foreach ($aco_array as $aco_section_value => $aco_value_array) {
			M()->debug("ACO Section Value: $aco_section_value ACO VALUE: $aco_value_array");
			//showarray($aco_array);

			if (!is_array($aco_value_array)) {
				M()->debug('is_conflicting_acl(): Invalid Format for ACO Array item. Skipping...');
				continue;
				// return TRUE;
			}
			...
			foreach ($aro_array as $aro_section_value => $aro_value_array) {
				M()->debug("ARO Section Value: $aro_section_value ARO VALUE: $aro_value_array");

				if (!is_array($aro_value_array))
				{
					M()->debug('is_conflicting_acl(): Invalid Format for ARO Array item. Skipping...');
					continue;
					// return TRUE;
				}

				M()->debug("Search: ACO Section: $aco_section_value ACO Value: $aco_value_array ARO Section: $aro_section_value ARO Value: $aro_value_array");
				...
				if (is_array($axo_array) AND count($axo_array) > 0) {
					foreach ($axo_array as $axo_section_value => $axo_value_array) {
						M()->debug("AXO Section Value: $axo_section_value AXO VALUE: $axo_value_array");

						if (!is_array($axo_value_array)) {
							M()->debug('is_conflicting_acl(): Invalid Format for AXO Array item. Skipping...');
							continue;
							// return TRUE;
						}

						M()->debug("Search: ACO Section: $aco_section_value ACO Value: $aco_value_array ARO Section: $aro_section_value ARO Value: $aro_value_array AXO Section: $axo_section_value AXO Value: $axo_value_array");
						...
						if (is_array($conflict_result) AND !empty($conflict_result)) {
							...
							if (count($conflict_result) > 0) {
								...
								M()->debug("Conflict FOUND!!! ACL_IDS: ($conflicting_acls_str)");
								return TRUE;
							}
						}
					}
		...
		M()->debug('is_conflicting_acl(): No conflicting ACL found.');
		return FALSE;
	}/*}}}*/
	...
	function add_acl($aco_array, $aro_array, $aro_group_ids=NULL, $axo_array=NULL, $axo_group_ids=NULL, $allow=1, $enabled=1, $return_value=NULL, $note=NULL, $section_value=NULL, $acl_id=FALSE ) {/*{{{*/
		...
		if (count($aco_array) == 0) {
			M()->debug("Must select at least one Access Control Object");
			return false;
		}

		if (count($aro_array) == 0 AND count($aro_group_ids) == 0) {
			M()->debug("Must select at least one Access Request Object or Group");
			return false;
		}
		...
		if (!empty($section_value)
			...
			M()->debug("Section Value: $section_value DOES NOT exist in the database.");
			return false;
		}
		...
		if ($this->is_conflicting_acl($aco_array,$aro_array,$aro_group_ids,$axo_array,$axo_group_ids,array($acl_id))) {
			M()->debug("Detected possible ACL conflict, not adding ACL!");
			return false;
		}
		...
		if ($this->get_acl($acl_id) == FALSE) {
			if ( empty($section_value) ) {
				...
				if( !$this->get_object_section_section_id(NULL, $section_value, 'ACL') ) {
					...
					if ( empty($section_value) ) {
						M()->debug("No valid acl section found.");
						return false;
					} else {
						M()->debug("Using default section value: $section_value.");
					}
				}
			}
			...
			if (empty($acl_id)) {
				...
				if (empty($acl_id)) {
					M()->debug("ACL_ID generation failed!");
					return false;
				}
			}
			*/
			...
			if ($result) {
				M()->debug("Update completed without error, delete mappings...");
		...
		M()->debug("Insert or Update completed without error, insert new mappings.");
		...
		foreach (array('aco', 'aro', 'axo') as $map) {
			...
			foreach ($map_array as $section_value => $value_array) {
				M()->debug('Insert: '. strtoupper($map) .' Section Value: '. $section_value .' '. strtoupper($map) .' VALUE: '. $value_array);
				// $this->showarray ($aco_value_array);

				if (!is_array($value_array)) {
					M()->debug('add_acl (): Invalid Format for '. strtoupper ($map) .' Array item. Skipping...');
					continue;
					// return true;
				}
				...
				foreach ($value_array as $value) {
					...
					if (empty($object_id))
					{
						M()->debug('add_acl(): '. strtoupper($map) . " Object Section Value: $section_value Value: $value DOES NOT exist in the database. Skipping...");
						$this->db->RollBackTrans();
						return false;
					}
		...
		foreach (array('aro', 'axo') as $map) {
			...
			foreach ($map_group_ids as $group_id) {
				M()->debug('Insert: '. strtoupper($map) .' GROUP ID: '. $group_id);

				$group_data = &$this->get_group_data($group_id, $map);

				if (empty($group_data)) {
					M()->debug('add_acl(): '. strtoupper($map) . " Group: $group_id DOES NOT exist in the database. Skipping...");
					$this->db->RollBackTrans();
					return false;
				}
	...
	function edit_acl($acl_id, $aco_array, $aro_array, $aro_group_ids=NULL, $axo_array=NULL, $axo_group_ids=NULL, $allow=1, $enabled=1, $return_value=NULL, $note=NULL, $section_value=NULL) {/*{{{*/
		...
		if (empty($acl_id) ) {
			M()->debug("Must specify a single ACL_ID to edit");
			return false;
		}
		if (count($aco_array) == 0) {
			M()->debug("Must select at least one Access Control Object");
	...
	function del_acl($acl_id) {/*{{{*/
		...
		if (empty($acl_id) ) {
			M()->debug("ACL_ID ($acl_id) is empty, this is required");
			return false;
		}
		...
		if (isset($sorted_groups[$root_id])) {
			...
			foreach ($sorted_groups[$root_id] as $id => $name) {
				...
				if (isset($sorted_groups[$id]) ) {
					//M()->debug("Recursing! Level: $level");
					$formatted_groups = $this->format_groups($sorted_groups, $type, $id, $next, $formatted_groups);
				} else {
					//M()->debug("Found last branch!");
				}
			}
		}

		//M()->debug("Returning final array.");
	...
	function get_group_id($value = NULL, $name = NULL, $group_type = 'ARO') {/*{{{*/
		...
		M()->debug("Value: $value, Name: $name, Type: $group_type" );
		...
		if (empty($name) AND empty($value) ) {
			M()->debug("name and value, at least one is required");
			return false;
		}
		...
		if ($row_count == 0) {
			M()->debug("Returned $row_count rows");
			return false;
		}
	...
	function get_group_children($group_id, $group_type = 'ARO', $recurse = 'NO_RECURSE') {/*{{{*/
		M()->debug("Group_ID: $group_id Group Type: $group_type Recurse: $recurse");
		...
		if (empty($group_id)) {
			M()->debug("ID ($group_id) is empty, this is required");
			return FALSE;
		}
	...
	function get_group_data($group_id, $group_type = 'ARO') {/*{{{*/
		...
		M()->debug("Group_ID: $group_id Group Type: $group_type");
	...
	function get_group_parent_id($id, $group_type='ARO') {/*{{{*/
		...
		M()->debug("ID: $id Group Type: $group_type");
		...
		if (empty($id) ) {
			M()->debug("ID ($id) is empty, this is required");
			return false;
		}
	...
	function get_root_group_id($group_type='ARO') {/*{{{*/
		...
		M()->debug('get_root_group_id(): Group Type: '. $group_type);
		...
		switch (strtolower($group_type)) {
			...
			case 'aro':
				...
				break;
			default:
				M()->debug('get_root_group_id(): Invalid Group Type: '. $group_type);
				return FALSE;
		}
		...
		switch ($row_count) {
			case 1:
				...
				return $row[0];
			case 0:
				M()->debug('get_root_group_id(): Returned 0 rows, you do not have a root group defined yet.');
				return FALSE;
		}

		M()->debug('get_root_group_id(): Returned '. $row_count .' rows, can only return one. Your tree is very broken.');
		return FALSE;
	}/*}}}*/
	...
	function add_group($value, $name, $parent_id=0, $group_type='ARO') {/*{{{*/
		...
		M()->debug("Name: $name Value: $value Parent ID: $parent_id Group Type: $group_type");

		$name = trim($name);
		$value = trim($value);

		if ( $name == '' ) {
			M()->debug("name ($name) OR parent id ($parent_id) is empty, this is required");
			return false;
		}
		...
		if ($parent_id == 0) {
			...
			if ($rs->RowCount() > 0) {
				M()->debug('add_group (): A root group already exists.');
				$this->db->RollBackTrans();
				return FALSE;
			}
			...
			if (empty($parent_id)) {
				M()->debug("parent id ($parent_id) is empty, this is required");
				$this->db->RollbackTrans();
				return FALSE;
			}
			...
			if (empty($row)) {
				M()->debug('add_group (): Parent ID: '. $parent_id .' not found.');
				$this->db->RollBackTrans();
				return FALSE;
			}
		...
		M()->debug('add_group (): Added group as ID: '. $insert_id);
		return $insert_id;
	}/*}}}*/
	...
	function get_group_objects($group_id, $group_type='ARO', $option='NO_RECURSE') {/*{{{*/
		...
		M()->debug("Group ID: $group_id");

		if (empty($group_id)) {
			M()->debug("Group ID:  ($group_id) is empty, this is required");
			return false;
		}
		...
		M()->debug("Got group objects, formatting array.");
	...
	function add_group_object($group_id, $object_section_value, $object_value, $group_type='ARO') {/*{{{*/
		...
		M()->info("Group ID: $group_id Section Value: $object_section_value Value: $object_value Group Type: $group_type");
		...
		if (empty($group_id) OR empty($object_value) OR empty($object_section_value)) {
			M()->warn("Group ID: ($group_id) OR Value ($object_value) OR Section value ($object_section_value) is empty, this is required");
			return false;
		}
		...
		//Group_ID == Member
		if ($row[1] == $row[2]) {
			M()->warn('add_group_object(): Object: ('. $object_section_value .' -> '. $object_value .') is already a member of Group: ('. $group_id .')');
		...
		M()->info( "ejecutando consulta: $query" );
		...
		if (!is_object($rs)) {
			...
			M()->error( 'error al ejecutar '. $query. ': '. $this->db->ErrorMsg() .' ('. $this->db->ErrorNo() .')' );
			return FALSE;
		}

		M()->info('add_group_object(): Added Object: '. $object_id .' to Group ID: '. $group_id);
	...
	function del_group_object($group_id, $object_section_value, $object_value, $group_type='ARO') {/*{{{*/
		...
		M()->debug("Group ID: $group_id Section value: $object_section_value Value: $object_value");

		$object_section_value = trim($object_section_value);
		$object_value = trim($object_value);

		if (empty($group_id) OR empty($object_value) OR empty($object_section_value)) {
			M()->debug("Group ID:  ($group_id) OR Section value: $object_section_value OR Value ($object_value) is empty, this is required");
			return false;
		}

 		if (!$object_id = $this->get_object_id($object_section_value, $object_value, $group_type)) {
			M()->debug("Group ID ($group_id) OR Value ($object_value) OR Section value ($object_section_value) is invalid. Does this object exist?");
			return FALSE;
		}
		...
		M()->debug("Deleted Value: $object_value to Group ID: $group_id assignment");
	...
	function edit_group($group_id, $value=NULL, $name=NULL, $parent_id=NULL, $group_type='ARO') {/*{{{*/
		M()->debug("ID: $group_id Name: $name Value: $value Parent ID: $parent_id Group Type: $group_type");
		...
		if (empty($group_id) ) {
			M()->debug('edit_group(): Group ID ('. $group_id .') is empty, this is required');
			return FALSE;
		}

		if ( !is_array($curr = $this->get_group_data($group_id, $group_type)) ) {
			M()->debug('edit_group(): Invalid Group ID: '. $group_id);
			return FALSE;
		}
		...
		if (!empty($parent_id)) {
			if ($group_id == $parent_id) {
				M()->debug('edit_group(): Groups can\'t be a parent to themselves. Incest is bad. ;)');
				return FALSE;
			}
			...
			if (is_array($children_ids)) {
				if (@in_array($parent_id, $children_ids) ) {
					M()->debug('edit_group(): Groups can\'t be re-parented to their own children, this would be incestuous!');
					return FALSE;
				}
			}
			unset($children_ids);
			...
			if (!$this->get_group_data($parent_id, $group_type)) {
				M()->debug('edit_group(): Parent Group ('. $parent_id .') doesn\'t exist');
				return FALSE;
			}
		}
		...
		if (empty($set)) {
			M()->debug('edit_group(): Nothing to update.');
			return FALSE;
		}
		...
		M()->debug('edit_group(): Modified group ID: '. $group_id);
	...
	function rebuild_tree($group_type = 'ARO', $group_id = NULL, $left = 1) {/*{{{*/
		M()->debug("Group Type: $group_type Group ID: $group_id Left: $left");
		...
		if (!isset($group_id)) {
			if ($group_id = $this->get_root_group_id($group_type)) {
				$left = 1;
				M()->debug('rebuild_tree(): No Group ID Specified, using Root Group ID: '. $group_id);
			} else {
				M()->debug('rebuild_tree(): A Root group could not be found, are there any groups defined?');
				return FALSE;
			}
		}
		...
		if ($rebuilt === FALSE) {
			M()->debug('rebuild_tree(): Error rebuilding tree!');
			$this->db->RollBackTrans();
			return FALSE;
		}

		$this->db->CommitTrans();
		M()->debug('rebuild_tree(): Tree rebuilt.');
		return TRUE;
	}/*}}}*/
	/**
	 * _rebuild_tree ()
	...
	function _rebuild_tree($table, $group_id, $left = 1) {/*{{{*/
		M()->debug("Table: $table Group ID: $group_id Left: $left");
	...
	function del_group($group_id, $reparent_children=TRUE, $group_type='ARO') {/*{{{*/
		...
		M()->debug("ID: $group_id Reparent Children: $reparent_children Group Type: $group_type");
		...
		if ($parent_id == 0) {
			...
			if (($child_count > 1) AND $reparent_children) {
				M()->debug('del_group (): You cannot delete the root group and reparent children, this would create multiple root groups.');
				$this->db->RollbackTrans();
				return FALSE;
			}
		}
		...
		M()->debug("deleted group ID: $group_id");
		$this->db->CommitTrans();
	...
	function get_object($section_value = null, $return_hidden=1, $object_type=NULL) {/*{{{*/
		...
		switch(strtolower(trim($object_type))) {
			...
			case 'acl':
				...
				break;
			default:
				M()->debug('get_object(): Invalid Object Type: '. $object_type);
				return FALSE;
		}

		M()->debug("Section Value: $section_value Object Type: $object_type");
	...
	function get_ungrouped_objects($return_hidden=1, $object_type=NULL) {/*{{{*/
		   ...
		   switch(strtolower(trim($object_type))) {
				   ...
				   case 'axo':
						   ...
						   break;
				   default:
						   M()->debug('get_ungrouped_objects(): Invalid Object Type: '. $object_type);
						   return FALSE;
		   }

		   M()->debug("Object Type: $object_type");
	...
	function get_objects($section_value = NULL, $return_hidden = 1, $object_type = NULL) {/*{{{*/
		switch (strtolower(trim($object_type))) {
			...
			case 'axo':
				...
				break;
			default:
				M()->debug('get_objects(): Invalid Object Type: '. $object_type);
				return FALSE;
		}
	...
	function get_object_data($object_id, $object_type=NULL) {/*{{{*/
		...
		switch(strtolower(trim($object_type))) {
			...
			case 'axo':
				...
				break;
			default:
				M()->debug('get_object_data(): Invalid Object Type: '. $object_type);
				return FALSE;
		}

		M()->debug("Object ID: $object_id Object Type: $object_type");

		if (empty($object_id) ) {
			M()->debug("Object ID ($object_id) is empty, this is required");
			return false;
		}

		if (empty($object_type) ) {
			M()->debug("Object Type ($object_type) is empty, this is required");
			return false;
		}
	...
	function get_object_id($section_value, $value, $object_type=NULL) {/*{{{*/
		...
		switch(strtolower(trim($object_type))) {
			...
			case 'axo':
				...
				break;
			default:
				M()->debug('get_object_id(): Invalid Object Type: '. $object_type);
				return FALSE;
		}

		M()->debug("Section Value: $section_value Value: $value Object Type: $object_type");

		$section_value = trim($section_value);
		$value = trim($value);

		if (empty($section_value) AND empty($value) ) {
			M()->debug("Section Value ($value) AND value ($value) is empty, this is required");
			return false;
		}
	...
	function get_object_section_value($object_id, $object_type=NULL) {/*{{{*/
		...
		switch(strtolower(trim($object_type))) {
			...
			case 'axo':
				...
				break;
			default:
				M()->debug('get_object_section_value(): Invalid Object Type: '. $object_type);
				return FALSE;
		}
	...
	function get_object_groups($object_id, $object_type = 'ARO', $option = 'NO_RECURSE') {/*{{{*/
		M()->debug('get_object_groups(): Object ID: '. $object_id .' Object Type: '. $object_type .' Option: '. $option);
		...
		switch(strtolower(trim($object_type))) {
			...
			case 'aro':
				...
				break;
			default:
				M()->debug('get_object_groups(): Invalid Object Type: '. $object_type);
				return FALSE;
		}

		if (empty($object_id)) {
			M()->debug('get_object_groups(): Object ID: ('. $object_id .') is empty, this is required');
			return FALSE;
		}
	...
	function add_object($section_value, $name, $value=0, $order=0, $hidden=0, $object_type=NULL) {/*{{{*/
		...
		switch(strtolower(trim($object_type))) {
			...
			case 'axo':
				...
				break;
			default:
				M()->debug('add_object(): Invalid Object Type: '. $object_type);
				return FALSE;
		}

		M()->info("Section Value: $section_value Value: $value Order: $order Name: $name Object Type: $object_type");
		...
		if (empty($name) OR empty($section_value) ) {
			M()->error("name ($name) OR section value ($section_value) is empty, this is required");
			return false;
		}

		if (strlen($name) >= 255 OR strlen($value) >= 230 ) {
			M()->error("name ($name) OR value ($value) is too long.");
			return false;
		}

		if (empty($object_type) ) {
			M()->error("Object Type ($object_type) is empty, this is required");
			return false;
		}
		...
		if (!is_object($rs)) {
			...
			M()->error('Error al ejecutar '. $query );
			return FALSE;
		}
		...
		$query = 'INSERT INTO '. $table .' (id,section_value,value,order_value,name,hidden) VALUES(NULL,'. $this->db->quote($section_value) .','. $this->db->quote($value) .','. $order .','. $this->db->quote($name) .','. $hidden .')';
		$rs = $this->db->Execute($query);

		if (!is_object($rs)) {

			M()->error('Error al insertar: '. $query );
			return false;
		}

		$insert_id = $this->db->Insert_ID();

		M()->info('Objeto agregado satisfactoriamente con ID: '. $insert_id );
	...
	function edit_object($object_id, $section_value, $name, $value=0, $order=0, $hidden=0, $object_type=NULL) {/*{{{*/
		...
		M()->debug("ID: $object_id Section Value: $section_value Value: $value Order: $order Name: $name Object Type: $object_type");
		...
		if (empty($object_id) OR empty($section_value) ) {
			M()->debug("Object ID ($object_id) OR Section Value ($section_value) is empty, this is required");
			return false;
		}

		if (empty($name) ) {
			M()->debug("name ($name) is empty, this is required");
			return false;
		}
		...
		M()->debug('edit_object(): Modified '. strtoupper($object_type) .' ID: '. $object_id);

		if ($old[0] != $value OR $old[1] != $section_value) {
			M()->debug("Value OR Section Value Changed, update other tables.");
			...
			M()->debug('edit_object(): Modified Map Value: '. $value .' Section Value: '. $section_value);
		}
	...
	function del_object($object_id, $object_type=NULL, $erase=FALSE) {/*{{{*/
		...
		switch(strtolower(trim($object_type))) {
			...
			case 'axo':
				...
				break;
			default:
				M()->debug('del_object(): Invalid Object Type: '. $object_type);
				return FALSE;
		}

		M()->debug("ID: $object_id Object Type: $object_type, Erase all referencing objects: $erase");
		...
		if (empty($object)) {
			M()->debug('del_object(): The specified object ('. strtoupper($object_type) .' ID: '. $object_id .') could not be found.');
			$this->db->RollbackTrans();
			return FALSE;
		}
		...
		if ($erase) {
			...
			M()->debug("Erase was set to TRUE, delete all referencing objects");
		...
		if ( ( isset($acl_ids) AND !empty($acl_ids) ) OR ( isset($groups_ids) AND !empty($groups_ids) ) ) {
			...
			M()->debug("Can't delete the object as it is being referenced by GROUPs (".@implode($groups_ids).") or ACLs (".@implode($acl_ids,",").")");
			$this->db->RollBackTrans();
			return false;
		} else {
	...
	function get_object_section_section_id($name = NULL, $value = NULL, $object_type = NULL) {/*{{{*/
		M()->debug("Value: $value Name: $name Object Type: $object_type");
		...
		switch(strtolower(trim($object_type))) {
			...
			case 'acl':
				...
				break;
			default:
				M()->debug('get_object_section_section_id(): Invalid Object Type ('. $object_type . ')');
				return FALSE;
		}
		...
		if (empty($name) AND empty($value) ) {
			M()->debug('get_object_section_section_id(): Both Name ('. $name .') and Value ('. $value .') are empty, you must specify at least one.');
			return FALSE;
		}
		...
		if ($row_count > 1) {
			M()->debug('get_object_section_section_id(): Returned '. $row_count .' rows, can only return one. Please search by value not name, or make your names unique.');
			return FALSE;
		}

		// No rows returned, no matching section found
		M()->debug('get_object_section_section_id(): Returned '. $row_count .' rows, no matching section found.');
		return FALSE;
	}/*}}}*/
	...
	function add_object_section($name, $value=0, $order=0, $hidden=0, $object_type=NULL) {/*{{{*/
		...
		M()->debug("Value: $value Order: $order Name: $name Object Type: $object_type");
		...
		if (!is_object($rs)) {
			...
			M()->debug("Added object_section as ID: $insert_id");
			return $insert_id;
		}
	}/*}}}*/
	...
	function edit_object_section($object_section_id, $name, $value=0, $order=0, $hidden=0, $object_type=NULL) {/*{{{*/
		...
		switch(strtolower(trim($object_type))) {
			...
			case 'acl':
				...
				break;
			default:
				M()->debug('edit_object_section(): Invalid Object Type: '. $object_type);
				return FALSE;
		}

		M()->debug("ID: $object_section_id Value: $value Order: $order Name: $name Object Type: $object_type");
		...
		if (empty($object_section_id) ) {
			M()->debug("Section ID ($object_section_id) is empty, this is required");
			return false;
		}
		...
		if (!is_object($rs)) {
			...
			M()->debug("Modified aco_section ID: $object_section_id");

			if ($old_value != $value) {
				M()->debug("Value Changed, update other tables.");
				...
				if (!is_object($rs)) {
					...
					if (!empty($object_map_table)) {
						...
						if ( !is_object($rs) ) {
							...
							M()->debug("Modified ojbect_map value: $value");
	...
	function del_object_section($object_section_id, $object_type=NULL, $erase=FALSE) {/*{{{*/
		...
		M()->debug("ID: $object_section_id Object Type: $object_type, Erase all: $erase");
		...
		if($object_ids AND !$erase) {
			...
			M()->debug("Could not delete the section ($section_value) as it is not empty.");
			...
			if (!is_object($rs)) {
				...
				M()->debug("deleted section ID: $object_section_id Value: $section_value");
				return true;
			}
	...
	function get_section_data($section_value, $object_type=NULL) {/*{{{*/
		...
		switch(strtolower(trim($object_type))) {
			...
			case 'axo':
				...
				break;
			default:
				M()->debug('get_section_data(): Invalid Object Type: '. $object_type);
				return FALSE;
		}
		...
		if (empty($section_value) ) {
			M()->debug("Section Value ($section_value) is empty, this is required");
			return false;
		}
		...
		M()->debug("Section does not exist.");
		return false;
	}/*}}}*/
